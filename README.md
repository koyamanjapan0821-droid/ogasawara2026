# 小笠原2026 PWA V7

GitHub Pages公開用の旅のしおりアプリです。

## V5変更点

- 鳥図鑑の画像参照を外部URLから `assets/birds/*.jpg` に変更
- 各鳥3枚、合計36枚のローカル画像を参照
- Service Workerのキャッシュ名を更新
- V4.1のスポット追加キャンセル修正を維持
- キャラクターのドラッグ移動・ランダム移動を維持

## 画像配置

画像本体はZIPに含めていません。GitHub上で以下へ個別アップロードしてください。

- `assets/characters/`
- `assets/birds/`

## 更新方法

1. ZIPを解凍
2. 中身をリポジトリ直下へアップロード
3. 画像は既存の `assets/characters/` と `assets/birds/` に維持または追加
4. GitHub Pages反映後、ブラウザを強制更新

PWAキャッシュが残る場合は、iPhoneのホーム画面アイコンを削除して再追加してください。


## V5.1
- `assets/characters/rare_kohtaro.png` を低確率レアキャラとして追加。
- 12秒ごとに低確率判定、出現時間は5秒。
- 画像本体はZIPに含めず、GitHub上の同パス配置を前提。


## V7
- 新しい鳥の追加機能を追加。
- 既存鳥・追加鳥にカメラロール画像を追加可能。
- 追加データと写真は端末のlocalStorageに保存。
- V6のキャラ別セリフ、孝太郎5%出現、ドラッグ機能を維持。

## V7.1
- キャラクターをタップしてセリフ表示後、3秒で別キャラクター画像へ切替。
- rare_kohtaro も対象。タップ後3秒で通常キャラへ戻ります。
- レアキャラの特別演出なし・5秒表示・ドラッグ可能仕様は維持。


## V7.2
- Service Workerのキャッシュ名を更新し、V7.1のapp.jsが確実に反映されるよう修正。
- キャラタップ後3秒で別キャラへ切り替える処理を再調整。
- レア孝太郎もタップ後3秒で通常キャラへ戻る。
